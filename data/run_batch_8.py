...
def par_run_PHI(read_cov_pair):
    ...
    output_file = f"data/Rec_haps/rec_hap_{read}_{cov}x_ilp.fa"
    log_file = f"data/Rec_haps/rec_hap_{read}_{cov}x_ilp.log"
    ground_truth = f"Ground_truth/{read}.fasta"
    
    cmd = (
        f"PHI -m0 -q0 -R100 -k31 -w25 -t{running_threads} "
        f"-g data/MHC_49-MC_30.gfa -r {downsampled_read} "
