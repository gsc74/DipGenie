...
def process_log_file(read, cov):
    hap_id = f'rec_hap_{read}_{cov}x'
    ...
    if os.path.exists(log_file):
        ...
        return read, hap_id, [recomb_cnt, real_time, peak_rss, edit_distance, alignment_identity, minimizers, ilp_percentage, filtered_minimizers]
    else:
        # Handle missing logs gracefully
        return read, hap_id, [None, None, None, None, None, None, None, None]
...
with ThreadPoolExecutor() as executor:
    ...
    for read in reads:
        data[read] = {}
        for cov in coverage:
            futures.append(executor.submit(process_log_file, read, cov))

    # Collect results as they are completed
    for future in as_completed(futures):
        read, hap_id, result = future.result()
