...
def par_run_KAGE(read_cov_pair):
    read, cov = read_cov_pair
    downsampled_read = f"data/reads_downsampled/{read}_{cov}x.fastq"
    output_prefix = f"data/Rec_haps/rec_hap_{read}_{cov}x_KG"
    ...
    # Source the bashrc file, run KAGE, and append edit distance to the log
    cmd = (
...
# Use multiprocessing to run KAGE on different read-coverage pairs in parallel
pool = multiprocessing.Pool(nbatches)
pool.map(par_run_KAGE, read_cov_pairs)
pool.close()
pool.join()
