...
# Calculate maximum y-limits based only on IQP data
max_runtime = max(phi_runtime.max().max(), phi_iqp_no_relax_runtime.max().max())
max_rss = max(phi_rss.max().max(), phi_iqp_no_relax_rss.max().max())

# Plot configuration
fig, axes = plt.subplots(1, 2, figsize=(12, 4))

# Data to plot and corresponding y-axis labels
plot_data = [(phi_runtime, 'Runtime (hours)'), (phi_iqp_no_relax_runtime, '')]

# Titles for each subplot
titles = ['(C) IQP (with relaxation)', '(D) IQP (no relaxation)']
...
for ax, (data, ylabel), title in zip(axes.flatten(), plot_data, titles):
    ...
    # Set y-axis limits based solely on IQP data
...
# compute average speed up for IQP vs MIQP over all haplotypes
average_speedup = []
for j in range(len(phi_coverage)):
    for i in range(len(legend_labels)):
        average_speedup.append(phi_iqp_no_relax_runtime.iloc[i, j] / phi_runtime.iloc[i, j])
average_speedup = sum(average_speedup) / len(average_speedup)
print('Average speedup IQP vs MIQP: %.2f' % average_speedup)

# Save the figure
plt.savefig('phi_iqp_vs_phi_miqp.pdf', bbox_inches='tight', dpi=1200, format='pdf')
