    ...
    std::vector<std::vector<AnchorRec>> anchorsByHap)
{
    ...
    std::vector<std::vector<dp_entry>> dp_next_thread(num_threads);

    auto is_better = [](const dp_entry& cand, const dp_entry& cur) -> bool {
        return cand.value > cur.value ||
               (cand.value == cur.value && cand.pred_i < cur.pred_i) ||
               (cand.value == cur.value && cand.pred_i == cur.pred_i && cand.pred_j < cur.pred_j);
    };

    #pragma omp parallel num_threads(num_threads)
    {
        const int tid = omp_get_thread_num();
        auto& local_next = dp_next_thread[tid];
        ...
        for (int l = 0; l < L; ++l) {
            ...
            // reduce thread-local buffers into dp_next
            #pragma omp for schedule(static)
            for (std::size_t t = 0; t < szN; ++t) {
                dp_entry best = dp_next_thread[0][t];
                for (int th = 1; th < num_threads; ++th) {
                    const dp_entry& cand = dp_next_thread[th][t];
