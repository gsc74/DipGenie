    ...
    std::vector<std::vector<AnchorRec>> anchorsByHap)
{
    ...
    auto compact_p1_pool = [&](std::vector<dp_entry>& buf) {
        std::vector<std::deque<EdgeNode>> new_pools(num_threads);
        std::unordered_map<EdgeNode*, EdgeNode*> remap;
        remap.reserve(buf.size() * 2);

        std::function<EdgeNode*(EdgeNode*)> clone_chain = [&](EdgeNode* node) -> EdgeNode* {
            if (node == nullptr) return nullptr;

            auto it = remap.find(node);
            if (it != remap.end()) return it->second;

            EdgeNode* new_prev = clone_chain(node->prev);

            // Put all compacted live nodes into pool 0.
            new_pools[0].emplace_back(node->from, node->to, new_prev);
            EdgeNode* new_node = &new_pools[0].back();

            remap.emplace(node, new_node);
            return new_node;
        };

        for (auto& e : buf) {
            e.p1_tail = clone_chain(e.p1_tail);
        }

        p1_edge_pools.swap(new_pools);
    };

    auto compact_p2_pool = [&](std::vector<dp_entry>& buf) {
        std::vector<std::deque<EdgeNode>> new_pools(num_threads);
        std::unordered_map<EdgeNode*, EdgeNode*> remap;
        remap.reserve(buf.size() * 2);
        ...
        for (auto& e : buf) {
            e.p2_tail = clone_chain(e.p2_tail);
        }

        p2_edge_pools.swap(new_pools);
    };
    ...
    #pragma omp parallel num_threads(num_threads)
    {
        for (int l = 0; l < L; ++l) {
            ...
            #pragma omp single
            {
                ...
                // Compact every 1000 completed transitions.
                // After the swap, only chains reachable from dp_cur are still needed.
                if (((l + 1) % 1000) == 0) {
                    compact_p1_pool(dp_cur);
                    compact_p2_pool(dp_cur);
                }
            }
        }
    }
